...
export class Migration20260905065947_user_auth extends Migration {

  override name = 'Migration20260905065947_user_auth';

  override up(): void | Promise<void> {
    this.addSql(`create table \`user_auth\` (\`id\` integer not null primary key autoincrement, \`user_id\` bigint not null, \`provider\` text check (\`provider\` in ('google', 'github', 'discord')) not null, \`provider_user_id\` text not null, \`created_at\` datetime not null, \`updated_at\` datetime not null, \`deleted_at\` datetime null, constraint \`user_auth_user_id_foreign\` foreign key (\`user_id\`) references \`user\` (\`id\`));`);
    this.addSql(`create index \`user_auth_user_id_index\` on \`user_auth\` (\`user_id\`);`);
    this.addSql(`create unique index \`user_auth_provider_user_id_unique\` on \`user_auth\` (\`provider_user_id\`);`);
